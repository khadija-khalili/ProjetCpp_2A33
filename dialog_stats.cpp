...
void Dialog_Stats::statistiques(QVector<double>* ticks,QVector<QString> *labels)
{
    QSqlQuery qry;
    int i=0;
    qry.exec("SELECT NUM_EVENT FROM EVENEMENT");
    while (qry.next())
    {
...
void Dialog_Stats::MakeStat()
{
    ...
    A->setStackingGap(1);
    /***** Couleurs*****/
    A->setName("Les Nombre des personnes selon les ID");
    ...
    ui->plot->xAxis->setLabel("NUM_EVENT");
    ...
    ui->plot->yAxis->setLabel("NOMBRE_PERSONNES");
...
void Dialog_Stats::on_pushButton_Fermer_clicked()
{
    close();
}
