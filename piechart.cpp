...
QChartView* Reservation::piechart()
{

    QPieSeries *series = new QPieSeries();
    series->append("Reunion", 1);
    series->append("Evenement", 2);
    series->append("Travail", 3);
    series->append("Révision", 4);
    ...
    chart->setTitle(" gestion de types ");
...
int Reservation::G_PVS1()
{
    int c22=0;
    QSqlQuery qry22;
    qry22.exec("SELECT num FROM Reservation WHERE type='reunion'");
    while(qry22.next())
    {
        c22++;
    }
...
int Reservation::G_PVS2()
{
    int c21=0;
    QSqlQuery qry21;
    qry21.exec("SELECT num FROM Reservation WHERE type='event'");
    while(qry21.next())
    {
        c21++;
    }
...
int Reservation::G_PVS3()
{
    int c20=0;
    QSqlQuery qry20;
    qry20.exec("SELECT num FROM Reservation WHERE type='travail'");
    while(qry20.next())
    {
        c20++;
    }
...
int Reservation::G_PVS4()
{
    int c19=0;
    QSqlQuery qry19;
    qry19.exec("SELECT num FROM Reservation WHERE type='revision'");
    while(qry19.next())
    {
        c19++;
    }
